...
class TokenTextViewTextStorage: NSTextStorage {
	...
	func updateFormatting() {
		...
		self.beginEditing()
		// Edit range of entire string, or else only the current line will be updated.
		let nsString = backingStore.string as NSString
		self.edited(.editedAttributes, range: NSRange(location: 0, length: nsString.length), changeInLength: 0)
